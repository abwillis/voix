/*********************************************************************
  ...
  VoiX is a vocal elimination software and can eliminate vocals from
  ...
  * stdin/stdout support by Andy Willis (abwillis1@gmail.com)
...
int main(int argc, char *argv[]) {
    ...
    if (argc==1) {  /* Display Version and Usage */
        printf("VoiX Version %s\n(http://vocaleliminator.sourceforge.net)\n", VERSION);
        printf("\nUsage: %s <infile> - for stdin [outfile] - for stdout [options]\n", argv[0]);
    ...
    /* Had to move version below usage to get stdout check to not sigsegv - if argc==1 then argv[2] will point outside the array */
    /* Display version */
    if (*argv[2] == '-') {
        fprintf(stderr,"VoiX Version %s\n(http://vocaleliminator.sourceforge.net)\n", VERSION);
    } else {
        printf("VoiX Version %s\n(http://vocaleliminator.sourceforge.net)\n", VERSION);
    }
    ...
    if (*argv[2] == '-') {
        fprintf(stderr,"Input: %s ", infilename);
    } else {
        printf("Input: %s ", infilename);
    }

    if (argc>=4) LPCutoff = atof(argv[3]);
    else LPCutoff = 200.0;
    if (argc>=5) HPCutoff = atof(argv[4]);
    else HPCutoff = 8000.0;
    ...
    if (infileChunk.FormatTag == 1) {
        if (*argv[2] == '-') {
            fprintf(stderr,"(Windows PCM");
        } else {
            printf("(Windows PCM");
        }
    }
    if (infileChunk.FormatTag == 3) {
        if (*argv[2] == '-') {
            fprintf(stderr,"(IEEE float");
        } else {
            printf("(IEEE float");
        }
    }
    if (*argv[2] == '-') {
        fprintf(stderr,", %d Hz, %d bit, ", infileChunk.SampleRate, infileChunk.Bits);
    } else {
        printf(", %d Hz, %d bit, ", infileChunk.SampleRate, infileChunk.Bits);
    }
    if (infileChunk.Channels == 1) {
        if (*argv[2] == '-') {
            fprintf(stderr,"mono)\n");
        } else {
            printf("mono)\n");
    ...
    if (*argv[1] == '-') {
      if (infileHeader.Length == 0) {
        infileHeader.Length = 0xFFFFFFFF;
      }
    }
    DataSize = infileHeader.Length;
    tmpDataSize = 0;
    strcpy(infileHeader.ID,"RIFF");
    infileHeader.Length = tmpDataSize+36;
    fwrite(&infileHeader,sizeof(infileHeader),1,outfile);
    strcpy(WaveId,"WAVE");
    fwrite(WaveId,sizeof(WaveId),1,outfile);
    strcpy(infileHeader.ID,"fmt ");
    infileHeader.Length = 16;
    fwrite(&infileHeader,sizeof(infileHeader),1,outfile);
    fwrite(&infileChunk,sizeof(infileChunk),1,outfile);
    strcpy(infileHeader.ID,"data");
    infileHeader.Length = tmpDataSize;
    fwrite(&infileHeader,sizeof(infileHeader),1,outfile);
    ...
    if (*argv[2] == '-') {
        fprintf(stderr,"(i)Lowpass: %.1f Hz, Highpass: %.1f Hz\n", LPCutoff, HPCutoff);
    } else {
    ...
    for (i = 1; i <= DataSize/infileChunk.BlockAlign; i++) {
        ...
        if (feof(infile)) {
            if (*argv[2] == '-') {
                ...
                fprintf(stderr,"\nThis is not unexpected when using stdin.\n");
            } else {
                printf("\nError(11): EOF(End of file) of input file detected.\n");
            }
            fclose(infile);
        ...
        if (percentage<i*20/(DataSize/infileChunk.BlockAlign)) {
            ...
            if (*argv[2] == '-') {
                fprintf(stderr,".");
                if (percentage%5==0) fprintf(stderr,"%d%c",percentage*5,'%');
            } else {
                printf(".");
                if (percentage%5==0) printf("%d%c",percentage*5,'%');
            }
        }
        ...
        if (Count == kWindowSize) {
            ...
            for (k=0; k < kWindowSize/2; k++) {
                cBuf[k] = cFFTBuf[2*k];
                cBuf[kWindowSize-k-1] = cFFTBuf[2*k+1];
            }
            for (k=0; k < kWindowSize/kOverlapCount; k++) {
                ...
                HPInput[0] = HPInput[1];
                HPInput[1]=HPInput[2];
                LPOutput[0] = LPOutput[1];
                LPOutput[1]=LPOutput[2];
                HPOutput[0] = HPOutput[1];
                HPOutput[1]=HPOutput[2];
            ...
            if (*argv[2] == '-') {
            /* No Op for stdout */} else {
            fseek(outfile,4,0);
            fwrite(&tmpRIFFSize,sizeof(tmpRIFFSize),1,outfile);
            fseek(outfile,40,0);
            fwrite(&tmpDataSize,sizeof(tmpDataSize),1,outfile);
